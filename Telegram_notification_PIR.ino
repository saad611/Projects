...
char ssid[] = "WONG'S";     // your network SSID (name)
char password[] = "569gw0NG$2348*2019:)"; // your network key
...
String chat_id = "-574296924";
#define BOTtoken "2090820366:AAG97dYjPbe5Ae_75IDoTVx3wdjG0ThUrB0"  // your Bot Token (Get from Botfather)
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/start") {
      ...
      welcome += "/start: start\n";
      welcome += "/turnoff: switch off display\n";
      welcome += "/turnon: switch on display\n";
...
static esp_err_t start_avi() {
  ...
  strftime(strftime_buf, sizeof(strftime_buf), "%F %H.%M.%S.avi", &timeinfo);
...
static void setupinterrupts() {
  ...
  pinMode(PIRpin, INPUT_PULLUP) ; //INPUT_PULLDOWN);
...
String Wifi_strength(String val)
{
  int converted = val.toInt();
  String strength = "";
  int absoulute_val = abs(converted);
  if(absoulute_val<=30)
  {
   strength="Perfect"; 
  }

  else if(absoulute_val<=50)
  {
   strength="Perfect"; 
  }

  else if(absoulute_val<=60)
  {
   strength="Good"; 
  }

  else if(absoulute_val<=67)
  {
   strength="Okay"; 
  }

  else if(absoulute_val<=70)
  {
   strength="Bad"; 
  }

  else if(absoulute_val<=80)
  {
   strength="Warning"; 
  }
...
void setup() {
  ...
  esp_sleep_enable_ext0_wakeup((gpio_num_t)PIRpin,1); //1 = High, 0 = Low
  ...
  // Display static text
  display.println("Device: Wide_Bed_Room");
  display.display(); 
...
void loop() {
  ...
  if (minutes >= 0) {
   if (currentMillis - previousMillis >= interval) {
      ...
      String value_strength = Wifi_strength((String)WiFi.RSSI());
      ...
      if(screen_on==true)
      {
        ...
        display.setCursor(0, 0);
        // Display static text
        display.print("Device: Wide_Bed_Room");
      
        int val = analogRead(VOLTAGE_PIN);
        
        int voltage = ((val*3.3)/4095);
      
        display.setCursor(0,10);
        
        // Display static text
        display.print("Battery level [V] ");
        display.setTextSize(1);
        display.setTextColor(WHITE);
        ...
        // Display static text
        display.print("Wifi strength [dBm]: ");
        display.setTextSize(1);
        display.setTextColor(WHITE);
        display.print(value_strength);
...
void send_the_picture() {
  ...
  if (active_interupt) {
    ...
    String sent = bot.sendMultipartFormDataToTelegramWithCaption("sendPhoto", "photo", "img.jpg",
                  "image/jpeg", "User Request", chat_id, fb_length,
                  isMoreDataAvailable, getNextByte, nullptr, nullptr);
  }
...
void send_the_video() {
  ...
  String sent2 = bot.sendMultipartFormDataToTelegramWithCaption("sendDocument", "document", strftime_buf,
                 "image/jpeg", "clip!", chat_id, psram_avi_ptr - psram_avi_buf,
